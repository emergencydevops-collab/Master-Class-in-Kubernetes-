...
## Step-E00: What are Microservices?
- Understand what are microservices on  a very high level

## Step-E01: What are we going to learn in this section?
...
## Step-E02: Pre-requisite -1: AWS RDS Database, ALB Ingress Controller & External DNS

### AWS RDS Database
- We have created AWS RDS Database as part of section [E06-EKS-Storage-with-RDS-Database](/E06-EKS-Storage-with-RDS-Database/README.md)
- We even created a `externalName service: E01-MySQL-externalName-Service.yml` in our Kubernetes manifests to point to that RDS Database. 

### ALB Ingress Controller & External DNS
- We are going to deploy a application which will also have a `ALB Ingress Service` and also will register its DNS name in Route53 using `External DNS`
- Which means we should have both related pods running in our EKS cluster. 
- We have installed **ALB Ingress Controller** as part of section [E08-E01-ALB-Ingress-Install](/E08-ELB-Application-LoadBalancers/E08-E01-ALB-Ingress-Install/README.md)
- We have installed **External DNS** as part of section [E08-E06-E01-Deploy-ExternalDNS-on-EKS](/E08-ELB-Application-LoadBalancers/E08-E06-ALB-Ingress-ExternalDNS/E08-E06-E01-Deploy-ExternalDNS-on-EKS/README.md)
```
...
## Step-E03: Pre-requisite-2: Create Simple Email Service - SES SMTP Credentials
### SMTP Credentials
...
- Download the credentials and update the same for below environment variables which you are going to provide in kubernetes manifest `E04-NotificationMicroservice-Deployment.yml`
```
...
## Step-E04: Create Notification Microservice Deployment Manifest
...
## Step-E05: Create Notification Microservice SMTP ExternalName Service
```yml
apiVersion: v1
kind: Service
metadata:
...
## Step-E06: Create Notification Microservice NodePort Service
```yml
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  - port: 8096
    targetPort: 8096
```
## Step-E07: Update User Management Microservice Deployment Manifest with Notification Service Environment Variables. 
...
- Update in `E02-UserManagementMicroservice-Deployment.yml`
```yml
          ...
          - name: NOTIFICATION_SERVICE_PORT
            value: "8096"    
```
## Step-E08: Update ALB Ingress Service Kubernetes Manifest
...
## Step-E09: Deploy Microservices manifests
```
# Deploy Microservices manifests
kubectl apply -f V1-Microservices/
```

## Step-E10: Verify the Deployment using kubectl
```
# List Pods
kubectl get pods
...
## Step-E11: Verify Microservices health-status via browser
```
...
## Step-E14: Rollout New Deployment - Set Image Option
```
...
## Step-E15: Rollout New Deployment - kubectl Edit
```
...
## Step-E16: Rollout New Deployment - Update manifest & kubectl apply
```
